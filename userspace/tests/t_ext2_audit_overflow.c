...
int test_large_offset_handling(void)
{
    ...
    if (fd < 0) {
        syslog(LOG_ERR, "[t_ext2_audit_overflow] Failed to create test file: %s\n", strerror(errno));
        return 1;
    }
...
int test_near_uint32_boundary(void)
{
    ...
    syslog(LOG_INFO, "[t_ext2_audit_overflow]   Offset: 0x%08X (%u)\n", offset, offset);
    syslog(LOG_INFO, "[t_ext2_audit_overflow]   Nbyte:  0x%08X (%u)\n", nbyte, nbyte);
    syslog(LOG_INFO, "[t_ext2_audit_overflow]   Sum:    0x%08X (%u) - OVERFLOW OCCURRED\n", sum, sum);
...
int test_mixed_boundary_conditions(void)
{
    ...
    struct stat st;
    if (fstat(fd, &st) < 0) {
        syslog(LOG_ERR, "[t_ext2_audit_overflow] Failed to fstat file: %s\n", strerror(errno));
        close(fd);
        return 1;
    }
    ...
    if (st.st_size != expected_size_t) {
        syslog(LOG_ERR, "[t_ext2_audit_overflow] File size mismatch: expected %zu, got %ld\n", expected_size_t, st.st_size);
        return 1;
    }
...
int main(void)
{
    ...
    if (failures == 0) {
        ...
        syslog(LOG_ERR, "[t_ext2_audit_overflow] ❌ %d TEST(S) FAILED\n", failures);
        closelog();
        return 1;
    }
}
