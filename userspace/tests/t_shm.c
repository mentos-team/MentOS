...
int shm_write(void)
{
    ...
    key = ftok(path, id);
    if (key == -1) {
        syslog(LOG_ERR, "[t_shm] Failed to generate IPC key using ftok.");
        return EXIT_FAILURE;
    }
    ...
    if (shmid == -1) {
        syslog(LOG_ERR, "[t_shm] Failed to create shared memory segment using shmget.");
        return EXIT_FAILURE;
    }
    ...
    if (str == (char *)-1) {
        syslog(LOG_ERR, "[t_shm] Failed to attach shared memory segment using shmat.");
        return EXIT_FAILURE;
    }
    ...
    if (shmdt(str) < 0) {
        syslog(LOG_ERR, "[t_shm] Failed to detach shared memory segment using shmdt.");
        return EXIT_FAILURE;
    }
...
int shm_read(void)
{
    ...
    if (shmid == -1) {
        syslog(LOG_ERR, "[t_shm] Failed to create or access shared memory using shmget.");
        return EXIT_FAILURE;
    }
    ...
    if (str == (char *)-1) {
        syslog(LOG_ERR, "[t_shm] Failed to attach to shared memory segment using shmat.");
        return EXIT_FAILURE;
    }

    // Check if both hashes match.
    if (strncmp(str, message, strlen(message)) != 0) {
        syslog(LOG_ERR, "[t_shm] Data does not match.");
        syslog(LOG_ERR, "[t_shm] Expected : `%s`", message);
        syslog(LOG_ERR, "[t_shm] Found    : `%s`", str);
        return EXIT_FAILURE;
    }
    ...
    if (shmctl(shmid, IPC_RMID, NULL) == -1) {
        syslog(LOG_ERR, "[t_shm] Failed to mark shared memory segment for removal using shmctl.");
        return EXIT_FAILURE;
    }
...
int main(int argc, char *argv[])
{
    ...
    if (shm_write()) {
        syslog(LOG_ERR, "[t_shm] Function shm_write failed.");
        closelog();
        return EXIT_FAILURE;
    }
    if (shm_read()) {
        syslog(LOG_ERR, "[t_shm] Function shm_read failed.");
        closelog();
        return EXIT_FAILURE;
    }
    closelog();
