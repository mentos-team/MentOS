...
int test_mount_operational(void)
{
    ...
    if (fd < 0) {
        syslog(LOG_ERR, "[t_ext2_audit_mount_cache] Failed to open file - filesystem may not be mounted: %s\n", strerror(errno));
        return 1;
    }
...
int test_cache_under_load(void)
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (fd < 0) {
            syslog(LOG_ERR, "[t_ext2_audit_mount_cache] Failed to create file %d", i);
            return 1;
        }
        ...
        if (written != to_write) {
            syslog(LOG_ERR, "[t_ext2_audit_mount_cache] Write to file %d incomplete: %ld of %ld\n", i, written, to_write);
            return 1;
        }
    }
...
int test_cache_on_reads(void)
{
    ...
    int fd = open(TEST_FILE, O_CREAT | O_WRONLY | O_TRUNC, 0644);
    if (fd < 0) {
        syslog(LOG_ERR, "[t_ext2_audit_mount_cache] Failed to create file: %s\n", strerror(errno));
        return 1;
    }
...
int test_cache_lifecycle(void)
{
    ...
    for (int cycle = 0; cycle < 5; cycle++) {
        int fd = open(TEST_FILE, O_CREAT | O_WRONLY | O_TRUNC, 0644);
        if (fd < 0) {
            syslog(LOG_ERR, "[t_ext2_audit_mount_cache] Failed to open for write in cycle %d\n", cycle);
            return 1;
        }
        ...
        if (bytes != sizeof(write_data)) {
            syslog(LOG_ERR, "[t_ext2_audit_mount_cache] Read failed in cycle %d\n", cycle);
            return 1;
        }

        if (memcmp(write_data, read_data, sizeof(write_data)) != 0) {
            syslog(LOG_ERR, "[t_ext2_audit_mount_cache] Data mismatch in cycle %d\n", cycle);
            return 1;
        }
    }
...
int main(void)
{
    ...
    if (failures == 0) {
        ...
        syslog(LOG_ERR, "[t_ext2_audit_mount_cache] ❌ %d TEST(S) FAILED\n", failures);
        closelog();
        return 1;
    }
}
