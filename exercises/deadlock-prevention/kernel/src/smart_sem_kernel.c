...
void **kmmalloc(size_t num, size_t size)
{
    void **ptr = (void **)kmalloc(num * sizeof(void *));
    if (!ptr) {
        return NULL;
    }
...
void kmfree(void **ptr, size_t num)
{
    if (!ptr) {
        return;
    }
...
static bool_t sys_sem_try(int id)
{
    ...
    if (arr_available && mat_max && mat_alloc && mat_need && idx_map_task_struct) {
        init_deadlock_structures(arr_available, mat_max, mat_alloc, mat_need, idx_map_task_struct);
    } else {
    ...
    // Perform request.
    bool_t ret = false;
    switch (request(req_vec, current_task_idx, arr_available, mat_alloc, mat_need, n, m)) {
    case WAIT:
    case WAIT_UNSAFE:
        break;
    case SAFE:
...
int sys_sem_create()
{
    ...
    pr_debug("sys_sem_create() -> %d\n", id);
    return id;
}

int sys_sem_destroy(int id)
{
    pr_debug("sys_sem_destroy(%d)\n", id);
    if ((id < 0) || (id > SEM_MAX)) {
        return -1;
    }
...
{
    pr_debug("sys_sem_try_acquire(%d)\n", id);
    if ((id < 0) || (id > SEM_MAX)) {
        return -1;
    }
    if (!semaphores[id].used) {
